...
export default function Header({ scrollY, currentPage, onNavigate, currentUser, onUserUpdate }: HeaderProps) {
  ...
  }, [scrollY, lastScrollY, HERO_THRESHOLD])
  ...
  return (
    <>
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 px-4 sm:px-6 py-2 sm:py-3 font-geist ${
        isHeaderVisible ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0 pointer-events-none'
      }`}>
        <div
          className={`max-w-7xl mx-auto rounded-2xl py-2.5 px-4 sm:px-5 flex justify-between items-center transition-all duration-500 ${
            scrolled
        ...
        >
          ...
          <nav className="hidden md:flex items-center gap-6">
            {navLinks.map((link) => (
              <a
                ...
                className={`text-[13px] font-medium transition-all tracking-tight cursor-pointer ${
                  currentPage === link.page
                    ? 'text-[#B8860B]'
                    : scrolled ? 'text-black hover:text-[#B8860B]' : 'text-black hover:text-[#B8860B]'
                }`}
              >
                {link.label}
              </a>
          ...
          <div className="hidden md:flex items-center gap-4 relative">
            <a
              ...
              className={`text-[13px] font-bold px-4 py-2 rounded-xl transition-all tracking-tight cursor-pointer ${
                scrolled
                  ? 'bg-black text-white hover:bg-[#B8860B]'
                  : 'bg-black text-white hover:bg-[#B8860B]'
              }`}
            >
              PARTNER WITH US
            ...
            <button
              ...
              className="btn-premium-black group/btn text-[13px] font-semibold px-5 py-2.5 rounded-xl cursor-pointer flex items-center gap-1.5 focus:outline-none"
            >
              <span>BOOK NOW</span>
            ...
            {currentUser ? (
              <div className="relative">
                <button
                  ...
                  className="w-8.5 h-8.5 rounded-full bg-[#D4AF37] text-black flex items-center justify-center font-bold text-xs shadow-md border-2 border-white hover:scale-105 active:scale-95 transition-all cursor-pointer relative"
                >
              ...
              <button
                ...
                className={`w-9 h-9 rounded-full flex items-center justify-center border transition-all cursor-pointer ${scrolled ? 'border-black/20 text-black/70 hover:text-[#D4AF37] hover:border-[#D4AF37]/50' : 'border-black/30 text-black/80 hover:text-[#D4AF37]'}`}
              >
          ...
          <div className="flex md:hidden items-center gap-2">
            {currentUser ? (
              <button
                ...
                className="w-7 h-7 rounded-full bg-[#D4AF37] text-black flex items-center justify-center font-bold text-[10px] shadow-sm"
              >
                {getUserInitials(currentUser.name)}
              </button>
            ) : (
              <button
                onClick={() => setIsLoginModalOpen(true)}
                className={`w-8 h-8 rounded-full flex items-center justify-center border transition-all cursor-pointer ${scrolled ? 'border-black/20 text-black/70 hover:text-[#D4AF37]' : 'border-black/30 text-black/80 hover:text-[#D4AF37]'}`}
              >
            ...
            <button
              ...
              className={`p-2 cursor-pointer focus:outline-none ${scrolled ? 'text-black/70 hover:text-black' : 'text-black/70 hover:text-[#D4AF37]'}`}
            >
        ...
        {/* Mobile Drawer — Dark Premium */}
        {isMobileMenuOpen && (
          <div className="md:hidden mt-2 bg-white/95 backdrop-blur-2xl border border-black/10 rounded-2xl shadow-xl shadow-black/10 p-4 flex flex-col gap-2 font-geist">
            {navLinks.map((link) => (
              <a
                key={link.label}
                href={`/${link.page === 'home' ? '' : link.page}`}
                ...
                className={`text-sm font-semibold py-2 px-3 rounded-xl transition-colors ${
                  currentPage === link.page ? 'text-[#D4AF37] bg-black/5' : 'text-black/70 hover:text-[#D4AF37] hover:bg-black/5'
                }`}
              >
                {link.label}
              </a>
            ...
            <button
              ...
              className="w-full text-center text-sm font-bold text-white bg-black hover:bg-[#D4AF37] hover:text-black py-3 rounded-xl shadow-md shadow-black/30 cursor-pointer border-0 focus:outline-none transition-colors"
            >
              Book Now →
            </button>
          </div>
      ...
      {isLoginModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          {/* Modal Content */}
          <div className="relative w-full max-w-md p-5 sm:p-8 rounded-[2rem] bg-black border border-white/10 shadow-2xl animate-in scale-in duration-300 text-white font-['Plus_Jakarta_Sans',_sans-serif] text-center overflow-hidden">
            {/* Ambient inner glow */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-64 bg-[#D4AF37]/15 rounded-full blur-3xl pointer-events-none" />
            ...
            <form onSubmit={handleAuthSubmit} className="space-y-4 text-left relative z-10">
              {authTab === 'signup' && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-2xl text-sm text-white placeholder-white/30 focus:outline-none focus:border-[#D4AF37]/60 focus:ring-2 focus:ring-[#D4AF37]/20 transition-all"
                  />
                </div>
              )}
              ...
              {authError && (
                <div className="p-3 bg-black border border-[#D4AF37]/50 text-[#D4AF37] rounded-2xl text-xs font-semibold">
                  ⚠️ {authError}
                </div>
              )}
              ...
              <button
                type="submit"
                className="w-full py-3.5 mt-2 bg-[#D4AF37] hover:bg-[#B8860B] text-black font-bold text-sm rounded-2xl transition-all shadow-lg shadow-[#D4AF37]/25 hover:shadow-[#D4AF37]/40 hover:-translate-y-0.5 active:translate-y-0 cursor-pointer border-0 focus:outline-none"
              >
